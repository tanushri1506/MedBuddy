...
def helper(predicted_disease):
    ...
    if not die.empty:
        ...
        die = ["No diets available."]

    wrkout = workout[workout['disease'] == predicted_disease]['workout']
    wrkout = wrkout.values.tolist() if not wrkout.empty else "No workout information available."
...
def predict():
    if request.method == 'POST':
        ...
        if invalid_symptoms:
            message = f"The symptoms you entered ({', '.join(invalid_symptoms)}) are not valid. Please select valid symptoms from the suggestions."
            return render_template('index.html', message=message)
    ...
    return render_template('index.html', message=None, predicted_disease=None)
